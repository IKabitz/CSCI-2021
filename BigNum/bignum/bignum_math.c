...
bool valid_input(char* input, int base) {  
  ...
  if(input[i] == '-') {
    fprintf(stderr, "encountered negative number.\n");
    return false;
  }
  
  while(input[i] >= '0') {

    // Handling standard numerical digits
    if(input[i] <= '9') {
      n = input[i] - '0';
    }

    //Handling upper alphabetics
    else if(is_upper_alphabetic(input[i])) {
      n = input[i] - '7';
    }

    //Handling lower alphabetics
...
/*
 * Subtract function
 * Used to subtract two bignum with the same sign
 */
int* subtract(int* input1, int* input2, int base, bool neg) {
  int len1 = bignum_length(input1);
  int len2 = bignum_length(input2);
  ...
  //Checking to see if result will be negative
  for(j = 0; j < len1; j++) {
    num1 = num1 * base + (input1[j]);
  }

  for(k = 0; k < len2; k++) {
    num2 = num2 * base + (input2[k]);
  }
  
  if((num1 < num2) || (len1 < len2)) isNegative = true;
  ...
  num2 = 0;
   ...
   /* For negative results, this process imitatates doing subtraction by hand
